...
public:
    ...
    bool begin(int address = MT333x_ADDR);
    ...
    bool sendMTKpacket(std::string command);
...
private:
    ...
    uint8_t _i2caddr;
    uint32_t _head; //Location of next available spot in the gpsData array. Limited to 255.
    uint32_t _tail; //Location of last spot read from gpsData array. Limited to 255.
};
