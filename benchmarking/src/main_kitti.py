...
our_t = [] # 1xlen of timestamps
our_p_gt = [] # 3x1 each sample
our_v_gt = [] # 3x1 each sample
our_ang_gt = [] # 3x1 each sample
our_u = [] # [ax, ay, az, gx, gy, gz]
...
class KITTIDataset(BaseDataset):
    ...
    def read_data(args):
        ...
        data_dir = os.listdir(args.path_data_base)
        ...
        for filename in data_dir:
            ...
            for row in reader:
                our_t.append(row[CSVIDXMAP["time"]]) # 1xlen of timestamps
                our_p_gt.append([row[CSVIDXMAP["gps_posx"], row[CSVIDXMAP["gps_posy"]], row[CSVIDXMAP["gps_posz"]]]]) # 3x1 each sample
                our_v_gt.append([row[CSVIDXMAP["gps_velx"], row[CSVIDXMAP["gps_vely"]], row[CSVIDXMAP["gps_velz"]]]]) # 3x1 each sample
                our_ang_gt.append([row[CSVIDXMAP["rx"]], row[CSVIDXMAP["ry"]], row[CSVIDXMAP["rz"]]]) # 3x1 each sample
                imu_data = [row[CSVIDXMAP["ax"]], row[CSVIDXMAP["ay"]], row[CSVIDXMAP["az"]], row[CSVIDXMAP["gx"]], row[CSVIDXMAP["gy"]], row[CSVIDXMAP["gz"]]]
                our_u.append(imu_data) # [ax, ay, az, gx, gy, gz]

            # our edits
            # convert from numpy
            t = torch.from_numpy(our_t)
            p_gt = torch.from_numpy(our_p_gt)
            v_gt = torch.from_numpy(our_v_gt)
            ang_gt = torch.from_numpy(our_ang_gt)
            u = torch.from_numpy(our_u)
...
class KITTIArgs():
        ...
        path_data_base = "../split_csv_benchmark_data"
        path_data_save = "../data"
        ...
        cross_validation_sequences = ['randomSat_Dec_5_14_10_58_2020_11']
        # test_sequences = ['2011_09_30_drive_0028_extract']
        test_sequences = ['randomSat_Dec_5_14_10_58_2020_3']
